...
public class entity {
    ...
    public static void do_funct(String cbor) throws CborException, IOException, ParseException {


        linking obj= new linking();
        obj.indexAllParas(cbor);



       /* String exampleText = "\"We have a long ride before us,\" Littlefinger pointed out. \"Eight days, maybe nine. And night is falling.\" \n" +
     ...
     HashMap<String, String> para_cont = new HashMap<String,String>();

        para_cont  = linking.para_cont;
        for (HashMap.Entry<String, String> entry : para_cont.entrySet()) {
            ArrayList<String> li= new ArrayList<String>();

            CoreDocument document = new CoreDocument(entry.getValue());
          //  System.out.println(entry.getValue());
            pipeline.annotate(document);
            for (CoreEntityMention em : document.entityMentions()) {
                String s= entity.Stopwords(em.toString());
                if(s.length()!=0){
                 ...
                 for(int i=0;i<list.size();i++){
                        ...
                        if(f==0 && em.entityType()!="NUMBER" && !s.equalsIgnoreCase("she")  && em.entityType().equalsIgnoreCase("PERSON")){
                           // System.out.println(s +"  "+em.entityType());
                            if(!li.contains(s)){
                                li.add(s);
            ...
            //    System.out.println(li);
                ...
                for (String value : li) {
                    ...
                    while (matcher.find()) {
                        ...
                        //    String t= entity.Stopwords(matcher.group().toString());
                        if (matcher.group().toString().toLowerCase().contains(value.toLowerCase())) {
                            //  System.out.println("query is :"+ li.get(i) +" "+ entity.Stopwords(matcher.group()).replace(li.get(i),""));
                            String query = value + " " + entity.Stopwords(matcher.group()).replace(value, "");
                            query = query.replaceAll("[^a-zA-Z0-9\\s+]", "");

                            //    System.out.println("query is "+ query);
                            obj.doSearch(query, 1);
                          //  System.out.println(value + "::::" + linking.name);
                            links.put(value,linking.name);
